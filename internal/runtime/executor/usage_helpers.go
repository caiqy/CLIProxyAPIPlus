...
// parseClaudeUsageNode extracts usage detail from a Claude usage JSON node.
//
// Token semantics:
//   - InputTokens  = input_tokens + cache_creation_input_tokens
//     (cache_creation tokens are billable input tokens written to the cache;
//     adding them prevents silent loss when cache_read is also present)
//   - CachedTokens = cache_read_input_tokens (tokens served from cache)
//     Falls back to cache_creation_input_tokens when cache_read is absent
//     and cache_creation is non-zero (first request that populates the cache).
//   - OutputTokens = output_tokens (tokens generated by the model)
//   - TotalTokens  = InputTokens + OutputTokens + ReasoningTokens
func parseClaudeUsageNode(usageNode gjson.Result) usage.Detail {
	cacheCreation := usageNode.Get("cache_creation_input_tokens").Int()
	detail := usage.Detail{
		InputTokens:  usageNode.Get("input_tokens").Int() + cacheCreation,
		OutputTokens: usageNode.Get("output_tokens").Int(),
		CachedTokens: usageNode.Get("cache_read_input_tokens").Int(),
	}
	if detail.CachedTokens == 0 && cacheCreation > 0 {
		// fall back to creation tokens when read tokens are absent
		detail.CachedTokens = cacheCreation
	}
...
func parseClaudeStreamUsage(line []byte) (usage.Detail, bool) {
	...
	usageNode := gjson.GetBytes(payload, "usage")
	if !usageNode.Exists() {
		// message_start events carry usage under message.usage
...
// accumulateClaudeStreamUsage merges partial usage from different Claude SSE
// events into a running total. message_start carries input/cache tokens while
// message_delta carries the final output_tokens.
func accumulateClaudeStreamUsage(accum *usage.Detail, detail usage.Detail) {
	if detail.InputTokens > accum.InputTokens {
		accum.InputTokens = detail.InputTokens
	}
	if detail.OutputTokens > accum.OutputTokens {
		accum.OutputTokens = detail.OutputTokens
	}
	if detail.CachedTokens > accum.CachedTokens {
		accum.CachedTokens = detail.CachedTokens
	}
	if detail.ReasoningTokens > accum.ReasoningTokens {
		accum.ReasoningTokens = detail.ReasoningTokens
	}
}

// publishAccumulatedClaudeUsage publishes the accumulated Claude usage if any
// tokens were collected.
func publishAccumulatedClaudeUsage(ctx context.Context, reporter *usageReporter, accum usage.Detail) {
	if accum.InputTokens == 0 && accum.OutputTokens == 0 && accum.CachedTokens == 0 && accum.ReasoningTokens == 0 {
		return
	}
	accum.TotalTokens = accum.InputTokens + accum.OutputTokens + accum.ReasoningTokens
	reporter.publish(ctx, accum)
}
